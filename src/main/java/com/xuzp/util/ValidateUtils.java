...
public class ValidateUtils {
    ...
    /** 验证 正整数A-正整数B */
    private static final String V_RANGE = "^([1-9]\\d*|0)[-~]([1-9]\\d*)$";
    ...
    public static boolean isValidPercent(String value) {
        if (!ValidateUtils.Posttive_float(value)) {
            return false;
        }
    ...
    public static boolean isValidRange(String value) {
        Pattern p  =  Pattern.compile(V_RANGE);
        Matcher m  =  p.matcher(value);
        if (m.matches()) {
            try {
                return new BigDecimal(m.group(1)).compareTo(new BigDecimal(m.group(2))) < 0;
            } catch(Exception e) {}
        }
        return false;
    }
